...
genrule(
    ...
    cmd = """
        ...
        CCC=`pwd`/external/org_linaro_components_toolchain_gcc_5_3_1_gnueabihf/bin/arm-linux-gnueabihf-
        CPU=$(TARGET_CPU)
        if [ "$${CPU}" == "bbb" ]; then
        ...
        CC=$(CC) make -C $${OPENSSL_ROOT} clean
        CC=$(CC) make -C $${OPENSSL_ROOT} depend -j8
        CC=$(CC) AR=$(AR) make -C $${OPENSSL_ROOT} -j8
